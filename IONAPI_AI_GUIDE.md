...
| **Total** | All modules combined | **275 KB** |
...
IonAPI v1.2.5 introduces specialized base classes for Paper and Folia to handle platform-specific logic natively.

### 📜 Paper (Standard)
For standard Paper servers (single-threaded):
```java
import com.ionapi.paper.IonPaperPlugin;

public class MyPlugin extends IonPaperPlugin {
    @Override
    public void onEnable() {
        super.onEnable(); // Initializes core services
        getLogger().info("Running heavily optimized on Paper!");
    }
}
```

### ⚡ Folia (Regionized Multithreading)
For Folia servers (parallel regions):
```java
import com.ionapi.folia.IonFoliaPlugin;

public class MyPlugin extends IonFoliaPlugin {
    @Override
    public void onEnable() {
        super.onEnable(); // Initializes core services
        getLogger().info("Running in parallel on Folia!");
    }
}
```

### Automatic Platform Detection
You can build a single JAR that works on both by shading IonAPI. The `IonPlugin` interface and `IonScheduler` abstract away the differences, so your core logic remains the same.
Note: You may need to use `IonPluginImpl` or a factory pattern if you want a true multi-platform single JAR, or simply release two versions.
...
## ⏱ Unified Scheduler (IonTasks)

IonAPI provides a unified `IonScheduler` that automatically handles the differences between Paper's main thread and Folia's region threading.

### Accessing the Scheduler
```java
IonScheduler scheduler = plugin.getScheduler();
```

### Basic Scheduling (Paper-like)
These methods work like the standard Bukkit scheduler. On Folia, they use the **Global Region Scheduler**.
```java
// Run now (Main Thread / Global Region)
scheduler.run(() -> getLogger().info("Running..."));

// Run later (20 ticks)
scheduler.runLater(() -> startEvent(), 20, TimeUnit.TICKS);

// Run timer (every second)
scheduler.runTimer(() -> tick(), 0, 1, TimeUnit.SECONDS);

// Async (Off-thread)
scheduler.runAsync(() -> database.save());
```

### 🌍 Context-Aware Scheduling (Folia Optimized)
**CRITICAL**: On Folia, you MUST schedule tasks on the correct region thread for entities or locations. These methods automatically delegate to the correct thread on Folia, and use the main thread on Paper.

```java
// Run on the thread owning the player/entity
scheduler.runAt(player, () -> {
    // Safe to modify player here
    player.teleport(spawn);
});

// Run on the thread owning a specific block location
scheduler.runAt(location, () -> {
    // Safe to modify block here
    location.getBlock().setType(Material.DIAMOND_BLOCK);
});

// Delayed context-aware task
scheduler.runAtLater(player, () -> {
    player.sendMessage("Welcome!");
}, 5, TimeUnit.SECONDS);

// Timer context-aware task (e.g., particle effect at location)
scheduler.runAtTimer(location, () -> {
    location.getWorld().spawnParticle(Particle.FLAME, location, 5);
}, 0, 100, TimeUnit.MILLISECONDS);
```

### 🔗 Task Chains (Fluent API)
...
// Note: For simple scheduling without chains, use plugin.getScheduler()
// See 'Unified Scheduler' section above.
```
...
Lightweight packet-based NPCs with skin support. Requires no external dependencies.
...
IonNPC npc = IonNPC.builder(plugin)
    ...
    .name("<red>Evil Guard")
    .skin("Notch")  // Fetches skin async from Mojang API
    .lookAtPlayer(true)
    .viewDistance(50)
    .persistent(true) // Saves to file
    .onClick(p -> {
        p.sendMessage("<red>Go away!");
...
### Managing NPCs
```java
// Hide/Show per player
npc.hide(player);
npc.show(player);

// Update
npc.teleport(newLocation);
npc.updateSkin("mattbaconz");
...
## 🔗 Cross-Server Messaging (ion-proxy)

Unified messaging interface for BungeeCord/Velocity and Redis.

### Setup
```java
// Plugin Messaging (Standard Bungee/Velocity channels)
IonMessenger messenger = IonProxy.pluginMessage(plugin);

// Redis Pub/Sub (Advanced - Requires Jedis)
IonMessenger redis = IonProxy.redis(plugin, "localhost", 6379, "password");
```

### Usage
```java
// Subscribe to a channel
messenger.subscribe("GlobalChat", msg -> {
    String content = msg.getData();
    // 'msg' also contains sender info if available
    Bukkit.broadcast(MiniMessage.miniMessage().deserialize(content));
});

// Broadcast to all servers listening
messenger.broadcast("GlobalChat", "<red>Alert: Server restarting!");

// Send to specific server (Plugin Messaging only)
messenger.send("Lobby-1", "KickPlayer", "Player123");
```

---

## 💉 Dependency Injection (ion-inject)

Lightweight DI container for cleaner plugin architecture. Eliminates singleton abuse.
...
public class MyPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        injector = IonInjector.create(this)
            .register(DatabaseService.class) // Auto-instantiated
            .register(ConfigService.class)
            // Register existing instance
            .register(EconomyService.class, new EconomyService(this))
            // Register factory
            .register(Gson.class, () -> new GsonBuilder().create())
            .build();
            
        // Create main command with dependencies injected
        MainCommand cmd = injector.create(MainCommand.class);
...
## 🏷️ Custom Placeholders (ion-placeholder)

Auto-registering PlaceholderAPI expansion wrapper.
...
public class MyPlaceholders implements IonPlaceholder {
    ...
    public @NotNull String getIdentifier() {
        return "myplugin"; // Usage: %myplugin_...%
    }
    ...
    public @Nullable String onRequest(@Nullable OfflinePlayer player, @NotNull String params) {
        ...
        return switch (params) {
            case "balance" -> String.valueOf(economy.getBalance(player));
            case "rank" -> perms.getRank(player);
            case "status" -> "Online";
            default -> null;
        };
    }
...
if (Bukkit.getPluginManager().isPluginEnabled("PlaceholderAPI")) {
    IonPlaceholderRegistry.register(new MyPlaceholders());
...
Unit test your plugin logic without a running Minecraft server.
...
class PlayerServiceTest {
    ...
    void setup() {
        ...
        db = new MockDatabase(); // custom mock
    }
    
    @Test
    void testSaveLoad() {
        PlayerService service = new PlayerService(plugin, db);
        UUID uuid = UUID.randomUUID();
        
        service.saveData(uuid, "Player1");
        
        Assertions.assertTrue(db.hasData("Player1"));
        Assertions.assertEquals("Player1", service.loadData(uuid).getName());
    }
}
```
